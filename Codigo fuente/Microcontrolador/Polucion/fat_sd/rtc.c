...
* Description    : pupulates time-struct based on counter-value
...
*                  account, HW always running in non-DST time
* Input          : None
* Output         : None
* Return         : not used
*******************************************************************************/
...
* Description    : inits HW RTC, 
*                  sets default timestamp if RTC has not been initialised before
* Input          : None
* Output         : None
* Return         : not used
*  Partly based on code from a STM RTC example (fwlib)
*******************************************************************************/
int rtc_init(void)
{
	volatile uint16_t i;
	...
	if (BKP_ReadBackupRegister(BKP_DR1_D) != 0xA5A5) {
		...
		/* Set initial value */
		RTC_SetCounter( (uint32_t)((11*60+55)*60) ); // here: 1st Jan. 2000 11:55:00
